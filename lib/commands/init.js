...
/**
 * Initialize SDD structure (called automatically during init)
 */
async function initSddStructure(projectRoot, purpose = null) {
  try {
    ...
    if (hasExisting) {
      console.log(chalk.dim('  ✓ Spec structure already exists'));
      return;
    }
    ...
    console.log(chalk.green('✓ Created telos/specs/ directories'));

    // Initialize config
    await sdd.initConfig(projectRoot);
    console.log(chalk.green('✓ Created telos/.telosrc.json'));

    // Create L4:purpose spec
    const projectName = path.basename(projectRoot);
    const projectPurpose = purpose || 'Define your project purpose here';
    ...
    console.log(chalk.green('✓ Created telos/specs/L4-purpose/purpose.md'));

    // Create TELOS.md entry point
    ...
    console.log(chalk.green('✓ Created telos/TELOS.md'));

    // Build initial index
    const index = await sdd.buildIndex(projectRoot);
    await sdd.saveIndex(projectRoot, index);
    console.log(chalk.green('✓ Created telos/index.json'));

  } catch (error) {
    console.log(chalk.yellow(`⚠ SDD structure setup skipped: ${error.message}`));
  }
}
...
async function initCommand(options) {
  ...
  try {
    ...
    console.log(chalk.cyan('\nInitializing spec-driven development structure...\n'));
    
    // Check if SDD already exists
    const specsPath = path.join(projectRoot, 'telos', 'specs');
    let sddExists = false;
    try {
      await fs.access(specsPath);
      ...
      const purposeAnswer = await inquirer.prompt([
        {
          type: 'input',
          name: 'purpose',
          message: 'What is the purpose of this project? (1-2 sentences)',
          default: 'Define your project purpose here'
        }
      ]);
      projectPurpose = purposeAnswer.purpose;
    }

    await initSddStructure(projectRoot, projectPurpose);
    ...
    const primaryPlatform = platforms.includes('claude') ? 'claude' : 
                           platforms.includes('opencode') ? 'opencode' : 'other';
    
    const platformName = primaryPlatform === 'claude' ? 'Claude Code' : 
                        primaryPlatform === 'opencode' ? 'OpenCode' : 
                        'your AI coding assistant';
    
    const initCmd = primaryPlatform === 'claude' ? '/telos:init' : 
                   primaryPlatform === 'opencode' ? '/telos-init' : 
                   '"Initialize Telos"';
    
    console.log(chalk.white(`1. Open this project in ${platformName}`));
    console.log(chalk.white(`2. Run: ${chalk.bold.green(initCmd)}`));
    console.log(chalk.white('3. Your AI will guide you through setup\n'));

    if (hasClaudeOrOpencode) {
      console.log(chalk.dim('Available slash commands:'));
      if (primaryPlatform === 'claude') {
        console.log(chalk.dim('  /telos:init               - Initialize with AI guidance'));
        console.log(chalk.dim('  /telos:quick              - Quick init (auto-accept)'));
        console.log(chalk.dim('  /telos:validate           - Validate specs and links'));
        console.log(chalk.dim('  /telos:status             - Show configuration'));
        console.log(chalk.dim('  /telos:sdd-discover       - Generate specs from code'));
        console.log(chalk.dim('  /telos:sdd-context        - Load context before changes'));
        console.log(chalk.dim('  /telos:sdd-generate-tests - Generate tests from scenarios\n'));
      } else {
        console.log(chalk.dim('  /telos-init               - Initialize with AI guidance'));
        console.log(chalk.dim('  /telos-quick              - Quick init (auto-accept)'));
        console.log(chalk.dim('  /telos-validate           - Validate specs and links'));
